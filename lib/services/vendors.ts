...
export const VendorsService = {
  async getVendors(params: {
    ...
    try {
      ...
      return await client
        .get(API_CONFIG.VENDORS.LIST, { searchParams })
  ...
  async getVendorById(id: string, businessId: string): Promise<VendorResponse> {
    ...
    try {
      return await client
        .get(API_CONFIG.VENDORS.GET.replace('{id}', id), {
  ...
  async createVendor(data: Record<string, unknown>): Promise<VendorResponse> {
    ...
    try {
      return await client
        .post(API_CONFIG.VENDORS.CREATE, { json: data })
        .json<VendorResponse>();
    } catch (error: unknown) {
      return handleServiceError(error);
    }
  ...
  async updateVendor(
    ...
    try {
      return await client
        .patch(API_CONFIG.VENDORS.UPDATE.replace('{id}', id), { json: data })
        .json<VendorResponse>();
    } catch (error: unknown) {
      return handleServiceError(error);
    }
  ...
  async deleteVendor(id: string, businessId: string): Promise<void> {
    const client = createAuthenticatedClient();
    try {
      await client.delete(API_CONFIG.VENDORS.DELETE.replace('{id}', id), {
        searchParams: { businessId },
      });
    } catch (error: unknown) {
      return handleServiceError(error);
