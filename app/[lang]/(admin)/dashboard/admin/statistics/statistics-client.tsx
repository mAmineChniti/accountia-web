...
export default function StatisticsClient({
  ...
  const formatPercent = (val: number | null | undefined) =>
    val != null ? `${(val * 100).toFixed(1)}%` : '0.0%';
    ...
    <div className="flex flex-col gap-6 p-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground mt-1">
            Real-time insights from MongoDB transactions (Auto-refresh every
            60s).
          </p>
        </div>
      </div>
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
        ...
        <div className="glass-card bg-card text-card-foreground flex flex-col gap-1 rounded-xl border p-6 shadow-sm">
          ...
          <div className="text-2xl font-bold">
            {formatCurrency(data.kpis.totalExpenditure)}
          </div>
        </div>
        ...
        <div className="glass-card bg-card text-card-foreground flex flex-col gap-1 rounded-xl border p-6 shadow-sm">
          ...
          <div className="text-2xl font-bold">
            {formatPercent(data.kpis.avgProfitMargin)}
          </div>
        </div>
        ...
        <div className="glass-card bg-card text-card-foreground flex flex-col gap-1 rounded-xl border p-6 shadow-sm">
          ...
          <div className="text-2xl font-bold">
            {data.kpis.successRate != null
              ? data.kpis.successRate.toFixed(1)
              : '0.0'}
            % ({data.kpis.totalTransactions || 0} tx)
          </div>
        </div>
        ...
        <div className="glass-card bg-card text-card-foreground flex flex-col gap-1 rounded-xl border p-6 shadow-sm">
          ...
          <div className="text-2xl font-bold">
            {formatScore(data.kpis.avgAccuracyScore)}
          </div>
        </div>
      </div>
      ...
      <div className="grid gap-6 md:grid-cols-2">
        {/* Line Chart */}
        <div className="glass-card bg-card text-card-foreground col-span-1 flex flex-col rounded-xl border p-6 shadow-sm md:col-span-2">
          <h3 className="mb-6 text-lg font-semibold">
            Revenue vs Expenditure vs Net Income
          </h3>
          ...
          <div className="h-[300px] w-full">
            <ResponsiveContainer width="100%" height="100%">
              <PieChart>
                ...
                >
                  {data.charts.profitMarginDistribution.map(
