...
export function VatReport({
  ...
  const exportPDF = () => {
    ...
    autoTable(doc, {
      head: [
        ['Invoice #', 'Date', 'Client', 'Net', 'VAT (19%)', 'Gross', 'Status'],
      ],
    ...
    doc.setFontSize(10);
    doc.text(
      `Taxable Revenue: ${report.totalTaxableRevenue.toLocaleString(undefined, { minimumFractionDigits: 2 })} TND`,
      14,
      finalY + 25
    );
    doc.text(
      `Output VAT Collected: ${report.totalOutputVat.toLocaleString(undefined, { minimumFractionDigits: 2 })} TND`,
      14,
      finalY + 32
    );
    doc.setFontSize(12);
    doc.setTextColor(138, 34, 34);
    doc.text(
      `Net VAT Payable: ${report.netVatPayable.toLocaleString(undefined, { minimumFractionDigits: 2 })} TND`,
      14,
      finalY + 42
    );
    ...
    <div className="w-full space-y-6 px-4 py-10 sm:px-6 lg:px-8">
      <div className="space-y-1">
        ...
        <p className="text-muted-foreground">
          Generate VAT reports for tax filing. Standard Tunisia VAT rate: 19%
        </p>
      </div>
      ...
      {report && (
        <>
            ...
            {[
              {
                ...
                value: report.totalTaxableRevenue,
                color: 'text-blue-600',
              },
              {
          ...
          <Card className="dark:bg-card/90 border-0 bg-white/90 shadow-sm">
            <CardHeader>
              <CardTitle>
                VAT Breakdown — {report.invoices.length} Invoice
                      ...
                      {report.invoices.map((inv) => (
                        ...
                        >
                          ...
                          <td className="py-2 text-right">
                            {inv.totalNet.toLocaleString(undefined, {
                              minimumFractionDigits: 2,
                            })}{' '}
                            TND
                          </td>
                          <td className="py-2 text-right text-orange-600">
                            {inv.totalVat.toLocaleString(undefined, {
                              minimumFractionDigits: 2,
                            })}{' '}
                            TND
                          </td>
                          <td className="py-2 text-right font-medium">
                            {inv.totalGross.toLocaleString(undefined, {
                              minimumFractionDigits: 2,
                            })}{' '}
                            TND
                          </td>
                          <td className="py-2">
                            <span className="rounded-full bg-green-100 px-2 py-0.5 text-xs text-green-700 capitalize">
                              {inv.status}
                            </span>
                          </td>
                        </tr>
                    ...
                    <tfoot>
                      <tr className="border-t-2 font-bold">
                        ...
                        <td className="pt-2 text-right">
                          {report.totalTaxableRevenue.toLocaleString(
                            undefined,
                            { minimumFractionDigits: 2 }
                          )}{' '}
                          TND
                        ...
                        <td className="pt-2 text-right text-orange-600">
                          {report.totalOutputVat.toLocaleString(undefined, {
                            minimumFractionDigits: 2,
                          })}{' '}
                          TND
                        </td>
                        <td className="pt-2 text-right">
                          {(
                            report.totalTaxableRevenue + report.totalOutputVat
